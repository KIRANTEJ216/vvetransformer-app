...
function getNavItems(role: string) {
  ...
  return [
    ...
    ...(isUser ? [{ href: "/quotations", label: "Quotes", icon: FileText }] : []),
    ...(isCeo
      ? [
...
export function BottomNav({ userRole }: { userRole: string }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-stroke bg-card lg:hidden shadow-[0_-2px_10px_rgba(0,0,0,0.08)]">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? "text-primary"
                  : "text-dark-5 hover:text-foreground"
              )}
            >
